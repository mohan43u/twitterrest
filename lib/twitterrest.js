...
var TwitterRest = function(options) {
	...
	var reqisstring = (typeof req == 'string');
	var turl = url.parse((reqisstring ? req : req.url), true);
	var headers = (reqisstring ? {} : req.headers);
	delete headers['host'];
	if(form) {
	...
	var clientrequest = (turl.protocol == 'https:' ? https : http).request({
	    ...
	    'port': turl.port,
	    'method': (reqisstring ? (form ? 'POST' : 'GET') : req.method),
	    'path': turl.path,
	    'headers': headers,
	    'agent': proxy
	});
	...
	if(turl.query.denied) {
	    ...
	    var post = {'oauth_token_verifier': params['oauth_token_verifier']};
	    req.method = 'POST';
	    req.url = url.format(turl);
	    var headers = {'Authorization': this.get_authorization(req.method,
								   req.url,
	...
	if(!turl.host
	   ...
	   || turl.host == this.twitter_userstream_host) {
		...
		req.on('end', function(body, req, form, callback) {
		    var post = querystring.parse(Buffer.concat(body).toString());
		    for(var k in form) {
			post[k] = form[k];
		    }
		    for(var k in post) {
		    ...
		    this.request(req, post, callback);
		}.bind(this, body, req, form, callback));
	    }
	    else {
		req.headers['Authorization'] = this.get_authorization(req.method, req.url, params);
		this.request(req, null, callback);
